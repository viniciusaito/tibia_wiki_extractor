...
from tibia_wiki_extractor.database.tables.legs import Legs
from sqlalchemy import delete

session = generate_database_session()
session.query(Legs).delete()

http_selector = generate_page_selector(url = 'https://www.tibiawiki.com.br/wiki/Calças')
...
for item in itens:
    for n, text in enumerate(item):
        if item[n] == '':
            item[n] = '0'
    data_to_be_inserted = Legs(name = item[0],
                                level = int(item[1]),
                                vocation = item[2],
                                slots = int(item[3]),
