...
from tibia_wiki_extractor.database.tables.shield import Shield
from sqlalchemy import delete

session = generate_database_session()
session.query(Shield).delete()

http_selector = generate_page_selector(url = 'https://www.tibiawiki.com.br/wiki/Escudos')
table = apply_css_filters_on_selector(http_selector, ['table#tabelaDPL'])
table = table[0]
table = apply_css_filters_on_selector(table, ['tr'])
column_list = {0:"name", 
               2:"level", 
               3:"vocation", 
               4:"slots", 
...
for item in itens:
    for n, text in enumerate(item):
        if item[n] == '':
            item[n] = '0'
    data_to_be_inserted = Shield(name = item[0],
                                level = int(item[1]),
                                vocation = item[2],
                                slots = int(item[3]),
                                defense = int(item[4]),
